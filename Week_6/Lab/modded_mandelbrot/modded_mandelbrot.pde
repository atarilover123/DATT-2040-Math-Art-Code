 ...
 * Simple rendering of the Mandelbrot set.
 ...
 modified to real time by Dan Tapper
 */
void setup() {
  size(640, 640);
  //noLoop();
...
void draw() {
  float w = map(mouseY, 0, height, 1, 10);
  //float w =4;
  ...
  for (int j = 0; j < height; j++) {
    ...
    for (int i = 0; i < width; i++) {
      ...
      while (n < maxiterations) {
        ...
        float twoab = map(mouseX,0,width,-10,10) * a * b;
      ...
      if (n == maxiterations) {
        ...
        pixels[i+j*width] = color(map(sqrt(norm), 0, 1, 0, 255),map(sqrt(norm), 0, 1, 0, 30),map(i, 0, width, 0, 100));
